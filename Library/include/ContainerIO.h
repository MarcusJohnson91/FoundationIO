#include "Macros.h"

#pragma  once

#ifndef  FoundationIO_ContainerIO_H
#define  FoundationIO_ContainerIO_H

#ifdef __cplusplus
extern "C" {
#endif
    
    /*!
     @header    ContainerIO.h
     @author    Marcus Johnson aka BumbleBritches57
     @copyright 2018+
     @version   1.0.0
     @brief     This header contains code for containers (type generic arrays)
     */
    
    /*!
     @enum                      ContainerTypes
     @constant                  UInteger8                  "Array of Unsigned Integers,  8 bits".
     @constant                  UInteger16                 "Array of Unsigned Integers, 16 bits".
     @constant                  UInteger32                 "Array of Unsigned Integers, 32 bits".
     @constant                  UInteger64                 "Array of Unsigned Integers, 64 bits".
     @constant                  SInteger8                  "Array of Signed Integers,    8 bits".
     @constant                  SInteger16                 "Array of Signed Integers,   16 bits".
     @constant                  SInteger32                 "Array of Signed Integers,   32 bits".
     @constant                  SInteger64                 "Array of Signed Integers,   64 bits".
     */
    typedef enum ContainerTypes {
                                UInteger8                  = 264,
                                UInteger16                 = 272,
                                UInteger32                 = 288,
                                UInteger64                 = 320,
                                SInteger8                  = 520,
                                SInteger16                 = 528,
                                SInteger32                 = 544,
                                SInteger64                 = 576,
    } ContainerTypes;
    
    typedef struct Container Container;
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    ContainerType "The type of the array containing the elements".
     @param    NumPlanes     "The number of dimensions in the array, for example a 4D array has 4 planes".
     @param    Width         "The number of elements on one row".
     @param    Height        "The number of elements on one column".
     */
    Container *Container_Init(ContainerTypes ContainerType, uint64_t NumPlanes, uint64_t Width, uint64_t Height);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     @param    Array2Attach  "A pointer to an array to attach to the container".
     */
    void       Container_Attach(Container *Collection, void ***Array2Attach);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     */
    uint8_t    Container_GetElementSizeInBytes(Container *Collection);
    
    /*!
     @abstract               "Returns the number of elements across all planes, rows, and columns".
     @param    Collection    "The container to access".
     */
    uint64_t   Container_GetSize(Container *Collection);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     */
    uint64_t   Container_GetNumPlanes(Container *Collection);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     @param    Width         "The width to set".
     */
    void       Container_SetWidth(Container *Collection, uint64_t Width);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     @param    Height        "The height to set".
     */
    void       Container_SetHeight(Container *Collection, uint64_t Height);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     */
    uint64_t   Container_GetWidth(Container *Collection);
    
    /*!
     @abstract               "Initalizes an empty container".
     @param    Collection    "The container to access".
     */
    uint64_t   Container_GetHeight(Container *Collection);
    
#ifdef __cplusplus
}
#endif

#endif   /* FoundationIO_CommandLineIO_H */
