...
extern "C" {
	...
	Audio2DContainer *Audio2DContainer_Init(Audio_Types Type, uint64_t NumChannels,
											Audio_ChannelMask *ChannelMap,
											uint64_t           SampleRate,
											uint64_t           NumSamples) {
		...
		if (NumSamples > 0) {
			...
			if (Audio != NULL) {
				...
				if (Array != NULL) {
					...
					Log(Log_DEBUG, __func__,
						UTF8String("Couldn't allocate the audio array"));
				}
				...
				Audio2DContainer_Deinit(Audio);
				Log(Log_DEBUG, __func__,
					UTF8String("Couldn't allocate the Audio2DContainer"));
			}
		} else if (NumSamples == 0) {
			Log(Log_DEBUG, __func__, UTF8String("NumSamples %llu is invalid"),
				NumSamples);
		}
		return Audio;
	}
	...
	void Audio2DContainer_Erase(Audio2DContainer *Audio) {
		if (Audio != NULL) {
			if (Audio->Type == (AudioType_Unsigned | AudioType_Integer8)) {
				...
				for (uint64_t Channel = 0ULL;
					 Channel < Audio2DContainer_GetNumChannels(Audio); Channel++) {
					for (uint64_t Sample = 0ULL; Sample < Audio->NumSamples; Sample++) {
	...
	Audio2DHistogram *Audio2DHistogram_GenerateHistogram(Audio2DContainer *Audio) {
		...
		if (Audio != NULL) {
			...
			if (Histogram != NULL) {
				...
				Log(Log_DEBUG, __func__,
					UTF8String("Couldn't allocate Audio2DHistogram"));
			}
		} else {
			Log(Log_DEBUG, __func__, UTF8String("Audio2DContainer Pointer is NULL"));
		}
			...
			if (SortAscending == Yes) { // Top to bottom
				if (Histogram->Type == (AudioType_Integer8 | AudioType_Unsigned)) {
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(uint8_t) Maximum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type ==
					(AudioType_Integer16 | AudioType_Unsigned)) {
					uint16_t *Audio = Audio2DHistogram_GetArray(Histogram);
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(uint16_t) Maximum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type ==
					(AudioType_Integer32 | AudioType_Unsigned)) {
					uint32_t *Audio = Audio2DHistogram_GetArray(Histogram);
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(uint32_t) Maximum(Audio[Element - 1], Audio[Element]);
						}
					}
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(int8_t) Maximum(Audio[Element - 1], Audio[Element]);
						}
					}
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(int16_t) Maximum(Audio[Element - 1], Audio[Element]);
						}
					}
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(int32_t) Maximum(Audio[Element - 1], Audio[Element]);
						}
					}
				}
			} else {
				if (Histogram->Type == (AudioType_Integer8 | AudioType_Unsigned)) {
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(uint8_t) Minimum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type ==
					(AudioType_Integer16 | AudioType_Unsigned)) {
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(uint16_t) Minimum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type ==
					(AudioType_Integer32 | AudioType_Unsigned)) {
					...
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(uint32_t) Minimum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type == (AudioType_Integer8 | AudioType_Signed)) {
					int8_t *Audio = Audio2DHistogram_GetArray(Histogram);
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(int8_t) Minimum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type == (AudioType_Integer16 | AudioType_Signed)) {
					int16_t *Audio = Audio2DHistogram_GetArray(Histogram);
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(int16_t) Minimum(Audio[Element - 1], Audio[Element]);
						}
					}
				} else if (Histogram->Type == (AudioType_Integer32 | AudioType_Signed)) {
					int32_t *Audio = Audio2DHistogram_GetArray(Histogram);
					for (uint64_t Core = 0ULL; Core < NumCores; Core++) {
						for (uint64_t Element = 1ULL;
							 Element < Histogram->NumEntries / NumCores; Element++) {
							Audio[Element - 1] =
								(int32_t) Minimum(Audio[Element - 1], Audio[Element]);
						}
					}
				}
			}
	...
	void Audio2DHistogram_Erase(Audio2DHistogram *Histogram) {
		if (Histogram != NULL) {
			if (Histogram->Type == (AudioType_Unsigned | AudioType_Integer8)) {
				...
				for (uint64_t Channel = 0ULL; Channel < Histogram->NumChannels;
					 Channel++) {
		...
		if (NumVectors > 0) {
			...
			if (Container != NULL) {
				...
				if (Container->Vectors != NULL) {
					...
					Log(Log_DEBUG, __func__,
						UTF8String("Couldn't allocate %lld AudioObjects"), NumVectors);
				}
			} else {
				Log(Log_DEBUG, __func__,
					UTF8String("Couldn't allocate Audio3DContainer"));
			}
		} else {
			Log(Log_DEBUG, __func__, UTF8String("NumVectors %lld is invalid"),
				NumVectors);
		}
		return Container;
	}

	void Audio3DContainer_SetVector(Audio3DContainer *Container,
									AudioVector *Vector, uint64_t Index) {
		if (Container != NULL && Vector != NULL && Index < Container->NumVectors) {
			...
			Log(Log_DEBUG, __func__, UTF8String("Index %llu is larger than %llu"),
				Index, Container->NumVectors);
		}
	}
	...
	ImageContainer *ImageContainer_Init(Image_Types       Type,
										Image_ChannelMask ChannelMask,
		...
		if (Width > 0 && Height > 0) {
			...
			if (Image != NULL) {
				...
				if (Array != NULL) {
					Image->Pixels = Array;
					Image->ChannelMap =
						calloc(NumViews * NumChannels, sizeof(Image_ChannelMask));
				...
				ImageContainer_Deinit(Image);
				Log(Log_DEBUG, __func__,
					UTF8String("Couldn't allocate the ImageContainer"));
			}
		} else if (Width == 0) {
	...
	void ImageContainer_SetChannelMap(ImageContainer *Image, uint64_t Index,
									  Image_ChannelMask ChannelMask) {
		if (Image != NULL) {
			uint8_t NumViews = ImageMask_GetNumViews(ChannelMask);
			uint8_t NumChannels = ImageMask_GetNumChannels(ChannelMask);
			if (NumViews == 1 && NumChannels == 1) {
				...
				Log(Log_DEBUG, __func__,
					UTF8String("ChannelMask must contain exactly 1 view and 1 channel"));
			}
		} else if (Image == NULL) {
	...
	uint8_t ImageMask_GetNumViews(Image_ChannelMask ChannelMask) {
		uint8_t           NumViews = 0;
		Image_ChannelMask JustViews =
			ChannelMask & (ImageMask_2D | ImageMask_3D_L | ImageMask_3D_R);
		do {
			if ((JustViews & 1) == 1) {
				NumViews += 1;
			}
	...
	uint64_t ImageMask_GetNumChannels(Image_ChannelMask ChannelMask) {
		uint64_t          NumChannels = 0ULL;
		Image_ChannelMask JustChannels =
			ChannelMask - (ImageMask_2D | ImageMask_3D_L | ImageMask_3D_R);
		do {
			if ((JustChannels & 1) == 1) {
				NumChannels += 1;
	...
	uint64_t ImageContainer_GetChannelsIndex(ImageContainer *Image,
											 Image_ChannelMask Mask) {
		uint64_t Index = 0ULL;
		if (Image != NULL) {
			Image_ChannelMask Channel =
				Mask - (ImageMask_2D | ImageMask_3D_L | ImageMask_3D_R);
			...
			if (NumViews == 1 && NumChannels == 1) {
				...
				Log(Log_DEBUG, __func__,
					UTF8String("Mask must contain 1 and only 1 view"));
			} else if (NumChannels != 1) {
				Log(Log_DEBUG, __func__,
					UTF8String("Mask must contain 1 and only 1 channel"));
			}
		} else {
			Log(Log_DEBUG, __func__, UTF8String("ImageContainer Pointer is NULL"));
		}
	...
	uint64_t ImageContainer_GetViewsIndex(ImageContainer *Image,
										  Image_ChannelMask Mask) {
		uint64_t Index = 0ULL;
		if (Image != NULL) {
	...
	uint64_t ImageContainer_GetAverage(ImageContainer *Image,
									   Image_ChannelMask ChannelMask) {
		...
		if (Image != NULL) {
			...
			if (View < NumViews - 1) {
				if (Channel < NumChannels - 1) {
					...
					Log(Log_DEBUG, __func__,
						UTF8String("You tried getting the average from a nonexistant "
								   "channel %lld"),
						Channel);
				}
			} else {
				Log(Log_DEBUG, __func__,
					UTF8String(
						"You tried getting the average from a nonexistant view %lld"),
					View);
			}
		} else {
			Log(Log_DEBUG, __func__, UTF8String("ImageContainer Pointer is NULL"));
	...
	uint64_t ImageContainer_GetMax(ImageContainer *Image,
								   Image_ChannelMask ChannelMask) {
		...
		if (Image != NULL) {
			...
			if (View < NumViews - 1) {
				if (Channel < NumChannels - 1) {
					...
					Log(Log_DEBUG, __func__,
						UTF8String("You tried getting the max from a nonexistant channel"));
				}
			} else {
				Log(Log_DEBUG, __func__,
					UTF8String(
	...
	uint64_t ImageContainer_GetMin(ImageContainer *Image,
								   Image_ChannelMask ChannelMask) {
	...
	void ImageContainer_Flip(ImageContainer *Image, FlipTypes FlipType) {
		if (Image != NULL) {
			uint8_t NumChannels = ImageContainer_GetNumChannels(Image);
			uint8_t NumViews = ImageContainer_GetNumViews(Image);
			if (FlipType == FlipType_Vertical ||
				FlipType == FlipType_VerticalAndHorizontal) {
				if (Image->Type == ImageType_Integer8) {
					uint8_t *Array = (uint8_t *) ImageContainer_GetArray(Image);
					for (uint64_t View = 0ULL; View < NumViews; View++) {
						for (uint64_t Width = 0ULL; Width < Image->Width; Width++) {
							for (uint64_t TopLine = 0ULL; TopLine < Image->Height; TopLine++) {
								for (uint64_t BottomLine = Image->Height; BottomLine > 0ULL;
									 BottomLine--) {
									for (uint64_t Channel = 0ULL; Channel < NumChannels;
										 Channel++) {
										uint8_t TopPixel = Array[View * Width * TopLine * Channel];
										uint8_t BottomPixel =
											Array[View * Width * BottomLine * Channel];
			...
			if (FlipType == FlipType_Horizontal ||
				FlipType == FlipType_VerticalAndHorizontal) {
				if (Image->Type == ImageType_Integer8) {
					uint8_t *Array = (uint8_t *) ImageContainer_GetArray(Image);
	...
	void ImageContainer_Resize(ImageContainer *Image, int64_t Left, int64_t Right,
			...
			if (New != NULL) {
				if (Type == ImageType_Integer8) {
					...
					if (Array != NULL) {
						for (uint8_t View = 0; View < NumViews; View++) {
							for (int64_t W = LeftOffset; W < RightOffset; W++) {
								for (int64_t H = TopOffset; H < BottomOffset; H++) {
									for (uint64_t Channel = 0ULL; Channel < NumChannels;
										 Channel++) {
										NewArray[View * (W - LeftOffset) * (H - TopOffset) *
											Channel] = Array[View * W * H * Channel];
									}
								}
							}
						}
						...
						Log(Log_DEBUG, __func__,
							UTF8String("Couldn't allocate an array for the cropped image"));
					}
					...
					if (Array != NULL) {
						for (uint8_t View = 1; View <= NumViews; View++) {
							for (int64_t W = LeftOffset; W < RightOffset; W++) {
								for (int64_t H = TopOffset; H < BottomOffset; H++) {
									for (uint64_t Channel = 1ULL; Channel <= NumChannels;
										 Channel++) {
										NewArray[View * (W - LeftOffset) * (H - TopOffset) *
											Channel] = Array[View * W * H * Channel];
									}
	...
	ImageContainer *ImageContainer_Compare(ImageContainer *Reference,
										   ImageContainer *Compare) {
		...
		if (Reference != NULL && Compare != NULL) {
			...
			if (RefType == ComType && RefNumViews == ComNumViews &&
				RefNumChannels == ComNumChannels && RefHeight == ComHeight &&
				RefWidth == ComWidth) {
				Difference =
					ImageContainer_Init(RefType, ImageContainer_GetChannelMask(Reference),
										RefWidth, RefHeight);
				if (Difference != NULL) {
					if (RefType == ImageType_Integer8 && ComType == ImageType_Integer8) {
						uint8_t ****RefArray =
							(uint8_t ****) ImageContainer_GetArray(Reference);
						uint8_t ****ComArray = (uint8_t ****) ImageContainer_GetArray(Compare);
						uint8_t ****DifArray =
							(uint8_t ****) ImageContainer_GetArray(Difference);
							...
							for (uint64_t Width = 0ULL; Width < RefWidth; Width++) {
								for (uint64_t Height = 0ULL; Height < RefHeight; Height++) {
									for (uint8_t Channel = 0; Channel < RefNumChannels; Channel++) {
										DifArray[View][Width][Height][Channel] =
											(RefArray[View][Width][Height][Channel] -
											 ComArray[View][Width][Height][Channel]) %
											0xFF;
									}
								}
							}
							   ...
							   ComType == ImageType_Integer16) {
						uint16_t ****RefArray =
							(uint16_t ****) ImageContainer_GetArray(Reference);
						uint16_t ****ComArray =
							(uint16_t ****) ImageContainer_GetArray(Compare);
						uint16_t ****DifArray =
							(uint16_t ****) ImageContainer_GetArray(Difference);
						for (uint8_t View = 0; View < RefNumViews; View++) {
							for (uint64_t Width = 0ULL; Width < RefWidth; Width++) {
								for (uint64_t Height = 0ULL; Height < RefHeight; Height++) {
									for (uint8_t Channel = 0; Channel < RefNumChannels; Channel++) {
				...
				Log(Log_DEBUG, __func__,
					UTF8String("Reference Type %d does not match Compare Type %d"),
					RefType, ComType);
			} else if (RefNumViews != ComNumViews) {
				Log(Log_DEBUG, __func__,
					UTF8String(
						"Reference NumViews %d does not match Compare NumViews %d"),
					RefNumViews, ComNumViews);
			} else if (RefNumChannels != ComNumChannels) {
				Log(Log_DEBUG, __func__,
					UTF8String(
						"Reference NumChannels %d does not match Compare NumChannels %d"),
					RefNumChannels, ComNumChannels);
			} else if (RefHeight != ComHeight) {
				Log(Log_DEBUG, __func__,
					UTF8String(
						"Reference Height %lld does not match Compare Height %lld"),
					RefHeight, ComHeight);
			} else if (RefWidth != ComWidth) {
				Log(Log_DEBUG, __func__,
					UTF8String("Reference Width %lld does not match Compare Width %lld"),
					RefWidth, ComWidth);
			}
	...
	void ImageContainer_Erase(ImageContainer *Image) {
		if (Image != NULL) {
			if (Image->Type == ImageType_Integer8) {
				...
				for (uint64_t View = 0ULL; View < ImageContainer_GetNumViews(Image);
					 View++) {
					for (uint64_t W = 0ULL; W < Image->Width; W++) {
						for (uint64_t H = 0ULL; H < Image->Height; H++) {
							for (uint64_t Channel = 0ULL;
								 Channel < ImageContainer_GetNumChannels(Image); Channel++) {
	...
	ImageHistogram *ImageHistogram_Init(ImageContainer *Image) {
		...
		if (Image != NULL) {
			...
			if (Histogram != NULL) {
				...
				if (Image->Type == ImageType_Integer8) {
					uint8_t ***HistogramArray =
						calloc(NumViews * NumChannels * NumPossibleColors, sizeof(uint8_t));
					if (HistogramArray != NULL) {
						...
						Log(Log_DEBUG, __func__,
							UTF8String("Couldn't allocate Histogram array"));
					}
				} else if (Image->Type == ImageType_Integer16) {
					uint16_t ***HistogramArray = calloc(
						NumViews * NumChannels * NumPossibleColors, sizeof(uint16_t));
					if (HistogramArray != NULL) {
						Histogram->Array = (void ***) HistogramArray;
						Histogram->Type = Image->Type;
					} else {
		...
		if (Histogram != NULL) {
			if (SortAscending == Yes) { // Top to bottom
				if (Histogram->Type == ImageType_Integer8) {
					...
					for (uint64_t View = 0ULL;
						 View < ImageMask_GetNumViews(Histogram->ChannelMask); View++) {
						for (uint64_t Width = 0ULL; Width < Histogram->Width; Width++) {
							for (uint64_t Height = 0ULL; Height < Histogram->Height; Height++) {
								for (uint64_t Channel = 1ULL;
									 Channel < ImageMask_GetNumChannels(Histogram->ChannelMask);
									 Channel++) {
									uint8_t PreviousChannel = *Image[View][Channel - 1];
									uint8_t CurrentChannel = *Image[View][Channel];
									*Image[View][Channel] =
										(uint8_t) Minimum(PreviousChannel, CurrentChannel);
								}
							}
						}
					}
						...
						for (uint64_t Width = 0ULL; Width < Histogram->Width; Width++) {
							for (uint64_t Height = 0ULL; Height < Histogram->Height; Height++) {
								for (uint64_t Channel = 1ULL;
									...
									*Image[View][Channel] =
										(uint16_t) Minimum(PreviousChannel, CurrentChannel);
								}
							}
						}
					}
